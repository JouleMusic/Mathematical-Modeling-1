...
def dbscan(allPoints, minDistance, minPointCount):
    ...
    for point in allPoints:
        ...
        #count = 0
    return noNoisePoints,otherPoints
...
if __name__ == '__main__':
    ...
    #imgxy=np.load('IMG500.npy')
    #imgxy=np.load('IMG45.npy')
    imgxy = list(imgxy)
    print(np.shape(imgxy))
    for i in range(len(imgxy)):
        imgxy[i]=list(imgxy[i])
    #print(imgxy)
    #noNoise,noise = dbscan(imgxy, 30, 155)
    noNoise,noise = dbscan(imgxy, 20, 223)
    print(np.shape(noNoise),'\n',np.shape(noise))
    np.save('noisexy.npy',noise)